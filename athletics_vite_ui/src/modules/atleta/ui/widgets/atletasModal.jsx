...
import { Shield, UserPlus, Mail, Lock, User, Phone, MapPin, CreditCard, Building, X as LucideX } from "lucide-react";
...
const InputField = ({ icon: Icon, id, label, ...props }) => (
  <div className="space-y-1 w-full">
    {label && <label htmlFor={id} className="text-sm font-semibold text-gray-700 dark:text-gray-300">{label}</label>}
...
const SelectField = ({ icon: Icon, id, label, children, ...props }) => (
  <div className="space-y-1 w-full text-gray-900 dark:text-gray-100">
    {label && <label htmlFor={id} className="text-sm font-semibold text-gray-700 dark:text-gray-300">{label}</label>}
    <div className="relative w-full">
      {Icon && <Icon className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={18} />}
      <select
...
const EditUserModal = ({ user, onClose, onUpdated }) => {
  ...
  const getSubmitButtonText = () => {
    ...
    return isCreateMode ? "Crear Atleta" : "Guardar Cambios";
  };
      ...
      <div className="relative z-10 bg-white dark:bg-[#1a1a1a] rounded-2xl w-full max-w-lg p-6 space-y-6 shadow-2xl border border-gray-200 dark:border-[#332122] transition-colors">
        ...
        <div className="flex justify-between items-center border-b border-gray-100 dark:border-[#332122] pb-4">
          <h3 className="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
            {isCreateMode ? <UserPlus className="text-[#b30c25]" aria-hidden="true" /> : <Shield className="text-[#b30c25]" aria-hidden="true" />}
        ...
        <form onSubmit={handleSubmit} className="space-y-4 max-h-[70vh] overflow-y-auto pr-2 custom-scrollbar">
          ...
          {/* Acciones */}
          <div className="flex justify-end gap-3 pt-4 border-t border-gray-100 dark:border-[#332122] sticky bottom-0 bg-white dark:bg-[#1a1a1a]">
            <button
              type="button"
              onClick={onClose}
...
EditUserModal.propTypes = {
  user: PropTypes.shape({
    ...
    tipo_estamento: PropTypes.string,
