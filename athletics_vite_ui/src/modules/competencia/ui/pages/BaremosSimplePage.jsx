...
import BaremoSimpleModal from "../widgets/BaremoSimpleModal";
import Swal from "sweetalert2";
import { Plus, Users, Activity, Edit2, Power, CheckCircle, List } from 'lucide-react';

const BaremosSimplePage = () => {
    ...
    const getPruebaName = (id) => {
        const p = pruebas.find(p => p.external_id === id || p.id === id);
    ...
    const toggleStatus = async (baremo) => {
        ...
        const result = await Swal.fire({
            title: '¿Estás seguro?',
            text: nuevoEstado
                ? `¿Desea activar el baremo?`
                : `¿Desea desactivar el baremo?`,
            icon: 'warning',
        ...
        try {
            await baremoService.update(baremo.external_id, {
                sexo: baremo.sexo,
                edad_min: baremo.edad_min,
                edad_max: baremo.edad_max,
                estado: nuevoEstado
            });
    ...
    const handleSubmit = async (data) => {
        try {
            if (selectedBaremo) {
                await baremoService.update(selectedBaremo.external_id, data);
                Swal.fire({
                    title: '¡Éxito!',
                    text: 'Baremo actualizado exitosamente',
                    icon: 'success',
                    confirmButtonColor: '#b30c25',
                    background: '#1a1a1a',
                    color: '#fff'
                ...
                // Para crear, necesitamos agregar al menos un item vacío
                await baremoService.create({
                    ...data,
                    items: [{ marca_minima: 0, marca_maxima: 0, clasificacion: "PRINCIPIANTE" }]
                });
                Swal.fire({
                    title: '¡Éxito!',
                    text: 'Baremo creado exitosamente',
                    icon: 'success',
                    confirmButtonColor: '#b30c25',
                    background: '#1a1a1a',
                    color: '#fff'
            ...
            Swal.fire({
                title: 'Error',
                text: err.message || 'Error al guardar el baremo',
                icon: 'error',
                confirmButtonColor: '#b30c25',
                background: '#1a1a1a',
                color: '#fff'
            ...
            <div className="max-w-6xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-6 mb-8">
                    <div className="space-y-1">
                        ...
                        <p className="text-gray-500 dark:text-gray-400">Vista simplificada - Información básica de baremos</p>
                    </div>
...
export default BaremosSimplePage;
