...
@auth_twofa_router_v1.post("/enable", response_model=Enable2FAResponse)
...
@auth_twofa_router_v1.post("/verify", response_model=MessageResponse)
async def verify_and_activate_2fa(
    ...
    if not current_user.totp_secret:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Primero debes iniciar el proceso con /enable"
        )
    ...
    return MessageResponse(message="2FA activado exitosamente. Ahora necesitarás un código en cada login.")


@auth_twofa_router_v1.post("/disable", response_model=MessageResponse)
async def disable_2fa(
    ...
    return MessageResponse(message="2FA deshabilitado exitosamente")


@auth_twofa_router_v1.post("/login", response_model=TokenPair)
...
async def login_with_2fa(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Token temporal inválido o expirado")
...
@auth_twofa_router_v1.post("/login-backup", response_model=TokenPair)
...
async def login_with_backup_code(
    ...
    if attempts > 3:
        ...
        raise HTTPException(
            status_code=status.HTTP_429_TOO_MANY_REQUESTS,
            detail="Demasiados intentos fallidos. Espera 30 minutos."
        )
